...
public class AutoCommands3 {
    ...
    public static Command RIGHT_SWIPE_OUTPOST(Drive drive, Superstructure superstructure, IntakeStructure intakeStructure, RobotState state) {
        return new SequentialCommandGroup(
                new ParallelDeadlineGroup(
                        followPathMirrored(AutoPaths.LS_CYCF, true, state),
                        new SequentialCommandGroup(
            ...
            return AutoBuilder.followPath(path)
                    .beforeStarting(
                            resetPose ?
                                    ...
                                    : Commands.none()
                    );//.until(() -> Math.abs(state.getEstimatedPose().getTranslation().getDistance(last)) < toleranceMeters);
        } catch (IOException e) {
