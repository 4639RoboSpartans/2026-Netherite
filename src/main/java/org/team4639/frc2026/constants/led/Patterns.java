...
public class Patterns {
  ...
  public static final LEDPattern SHOOTER_REQUESTED_AND_INTAKE =
      createMovingPattern(SHOOTER_REQUESTED);
  ...
  private static LEDPattern createMovingPattern(LEDPattern pattern) {
    return new MovingPattern(
        new BlockPattern(MOVING_BLOCK_SIZE, pattern, LEDPattern.BLANK), LEDS_PER_SECOND);
  }

  private static LEDPattern color(Color color) {
    return new RGBPattern(
        LEDPattern.ofColor(color),
        new Color8Bit(0, 63, 0),
        new Color8Bit(255, 0, 0),
        new Color8Bit(0, 0, 31));
  }
}
